...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  vector<float> line;
  float belief = 1/(float(grid.size())*grid[0].size());
  for (int i = 0; i<grid.size();i++){
    line.clear();
    for (int j = 0; j < grid[0].size(); j++){
      line.push_back(belief);
    }
  ...
  float blurring) 
{
  ...
  for (int i = 0; i < beliefs.size(); i++){
  	for (int j = 0; j < beliefs[0].size(); j++){
  		int temp_i = modmtx(i + dy, beliefs.size());
  		int temp_j = modmtx(j + dx, beliefs[0].size());
  		newGrid[temp_i][temp_j] = beliefs[i][j];
  		cout << temp_i << " "<< temp_j<<" "<<newGrid[temp_i][temp_j]<<endl;
  	}
  }
  // your code here
