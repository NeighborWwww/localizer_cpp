...
float summtx(vector<vector<float>> grid){
	float sum = 0;
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	vector < vector <float> > newGrid,window;
	newGrid = zeros(grid.size(),grid[0].size());
	float center = 1.0 - blurring;
	float edge = blurring/6.0;
	float corner = edge/2.0;
	cout<<"blurring process: "<<endl;
	window = {{corner, edge, corner},{edge, center, edge},{corner, edge, corner}};
	for (int i = 0; i < grid.size(); i++){
		for (int j = 0; j < grid[0].size(); j++){
			float grid_val = grid[i][j];
			for (int dx = -1; dx < 2; dx++){
				for (int dy = -1; dy < 2; dy++){
					newGrid[modmtx(i+dy,3)][modmtx(j+dx,3)] += window[dx+1][dy+1] * grid_val;
				}
			}
		}
	}

	cout<<"blurring end"<<endl;
